...
public class DecaITrambolina extends Application {
	...
	private class StariPeraC{
		...
		private int brDevojica = 0;
		
		private void pustiUnutra(Pol pol) throws InterruptedException {
			brava.lock();
			try {
				if(pol == Pol.MUSKI) {
					if(brDevojica>0) {
		...
		private void pustiNapolje(Pol pol) {
			brava.lock();
			try {
				if(pol == Pol.MUSKI) {
					brDecaka--;
	...
	private class StariPera{
		...
		public StariPera(int tezina,int broj) {
			this.maxBr = broj;
	...
	protected final int MAX_BR_DECE = 8;

	protected StariPera pera = new StariPera(MAX_TEZINA,MAX_BR_DECE);
	protected enum Pol {
		MUSKI, ZENSKI;
	}
	...
	protected final Operation odmaranje = duration("10±2").text("Одмара").textAfter("Чека");
